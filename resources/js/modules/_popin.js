...
export default class Popin extends module {
  ...
  scrollBehaviour (state) {
    ...
    Object.assign(document.style, { overflow: state ? 'hidden' : '' })
  }
  ...
  toggleEvents(state) {
    ...
    closeEls.forEach(element => {
      element[`${functionName}EventListener`]('touchstart', this.close, { passive: true })
      element[`${functionName}EventListener`]('click', this.close, { passive: true })
    });
  ...
  setFocusToFirstNode() {
    ...
    const nodesWhichAreNotCloseTargets = focusableNodes.filter(node => !node.hasAttribute('data-popin'))
